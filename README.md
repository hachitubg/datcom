...
### Bảo toàn dữ liệu database (rất quan trọng)

- File dữ liệu thật nằm ở `datcom.db` (trong thư mục project).
- Các bước pull/restart bên dưới **không xóa database**.
- Trước khi deploy, nên backup 1 bản:

```bash
cd /var/www/datcom
cp datcom.db datcom.db.backup-$(date +%F-%H%M%S)
```
...
### Tránh conflict khi tạo PR (khuyến nghị)

```bash
# Luôn tạo branch mới từ main mới nhất
cd /var/www/datcom
...
> Không nên dùng lại branch cũ cho nhiều PR liên tiếp.
